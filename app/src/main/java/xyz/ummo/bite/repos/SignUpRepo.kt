...
class SignUpRepo {
    ...
    fun signUserInDirectly(userSignUpDetails: JSONObject){
        val requestBody = userSignUpDetails.toString().toRequestBody(MEDIA_TYPE_JSON)
        val request = Request.Builder()
            .url("https://bite-api-dev.herokuapp.com/auth/login")
    ...
    fun signUserUpDirectly(userSignUpDetails: JSONObject) {
        ...
        try {
            ...
            try {
                if (responseCode == 201) {
                    Timber.e("SIGN-UP SUCCESSFUL -> $responseString")
                    registrationCallbackEvent.responseStatusCode = responseCode
                    EventBus.getDefault().post(registrationCallbackEvent)
                } else {
                    val responseMessage = JSONObject(responseString).getString("message")
...
fun signUserUpGoogle(authCode: String?)
{
    val request = authCode?.toRequestBody(MEDIA_TYPE_MARKDOWN)?.let {
        Request.Builder()
            .url("https://bite-api-dev.herokuapp.com/auth/google")
    ...
    if (request != null) {
        client.newCall(request).execute().use { response ->
            ...
            Timber.e("Google Sign-in -> ${response.body}")
        }
    }
}
